...
def test_point_forecast_policy_orders_forecast_plus_safety_stock():
    ...
    assert policy.order_quantity_for(state_period1) == 13
...
def test_point_forecast_policy_k_mae_fill_rate_still_uses_rmse_variability():
    kwargs = dict(
        forecast=[10, 10, 10, 10, 10, 10],
        actuals=[8, 13, 9, 12, 10, 10],
        lead_time=2,
    ...
    rmse_policy = PointForecastOptimizationPolicy(
        **kwargs,
        safety_stock_method="k_rmse",
    )
    mae_policy = PointForecastOptimizationPolicy(
        **kwargs,
        safety_stock_method="k_mae",
    )
    state_period3 = InventoryState(period=3, on_hand=0, on_order=0, backorders=0)

    assert mae_policy.order_quantity_for(state_period3) == rmse_policy.order_quantity_for(
        state_period3
    )
